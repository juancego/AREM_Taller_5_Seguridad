...
La seguridad en Zajana se basa en una arquitectura desplegada sobre Microsoft Azure que combina controles de identidad, protección de datos y monitoreo continuo. El acceso a la plataforma se asegura mediante autenticación multifactor (MFA) con Microsoft Authenticator y validación de tokens JWT gestionados por Entra ID, lo que garantiza que solo usuarios legítimos puedan interactuar con los sistemas. Los flujos de información están protegidos a través de Azure Front Door y Azure Firewall, que filtran y balancean el tráfico, mientras que Azure API Management (APIM) controla la exposición de servicios con validación de claims y limitación de solicitudes. Los datos sensibles se almacenan en Cosmos DB, SQL Database y Azure Storage, todos con cifrado en reposo y accesibles únicamente mediante Private Endpoints, reduciendo el riesgo de exposición pública. La capa de seguridad se refuerza con Defender for Cloud e Intune, que vigilan la integridad de dispositivos y aplicaciones, y con Azure Sentinel, que centraliza la correlación de eventos y alertas de seguridad. De esta manera, Zajana asume la responsabilidad de configurar y operar la seguridad de sus procesos críticos, mientras que Azure garantiza la disponibilidad, escalabilidad y cumplimiento técnico de los servicios en la nube.
